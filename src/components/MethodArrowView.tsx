...
export default function MethodArrowView() {
  ...
  // Get active problems (all problems, sorted by year)
  ...
  // Get active methods (leaf methods with targets only, not parent categories)
  const activeMethods = useMemo(() => {
    return methods.filter(m => m.targets.length > 0 && (!m.children || m.children.length === 0)).sort((a, b) => a.year - b.year)
  }, [methods])
    ...
    <div className="h-full w-full flex flex-col bg-zinc-950">
      ...
      <div className="flex items-center gap-3 px-5 py-3 border-b border-zinc-800/80 bg-zinc-900/40">
        ...
        <div className="flex items-center gap-1 ml-auto">
          ...
          <button onClick={() => { setZoom(0.85); setPan({ x: 50, y: 20 }) }} className="p-2 hover:bg-zinc-800 rounded-lg">
