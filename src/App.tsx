...
import { 
  ...
  Layers, Cpu, Eye, Zap, Filter, ChevronDown
} from 'lucide-react'
...
const DOMAINS = [
  { id: 'all', name: 'All Domains', color: '#6366f1', icon: Layers },
  { id: 'b_diffusion', name: 'Diffusion Policy', color: '#22c55e', icon: Zap },
  { id: 'b_vla', name: 'VLA Models', color: '#3b82f6', icon: Eye },
  { id: 'b_tactile', name: 'Tactile', color: '#f59e0b', icon: Cpu },
  { id: 'b_perception', name: 'Perception', color: '#8b5cf6', icon: Target },
  { id: 'b_fusion', name: 'Fusion', color: '#14b8a6', icon: Workflow },
  { id: 'b_policy', name: 'Policy', color: '#ec4899', icon: GitBranch },
  { id: 'b_manipulation', name: 'Manipulation', color: '#f97316', icon: Layers },
]

function App() {
  const { activeView, setActiveView, viewConfig, updateViewConfig, loadData, undo, redo, selectedNode, selectNode } = useAppStore()
  const { branches, loadData: nexusLoad } = useNexusStore()
      ...
      <div 
        ...
        style={{ 
          ...
          background: viewConfig.darkMode 
            ? `radial-gradient(circle, ${activeDomainColor}25 0%, transparent 70%)`
            : `radial-gradient(circle, ${activeDomainColor}15 0%, transparent 70%)`
        }} 
      />
      <div 
        className="ds-ambient-glow"
      ...
      <aside className={`${sidebarCollapsed ? 'w-16' : 'w-60'} flex flex-col shrink-0 transition-all duration-500 ds-spatial-sidebar z-20`}>
        ...
        <div className={`p-4 border-b ${viewConfig.darkMode ? 'border-white/5' : 'border-black/5'} flex items-center gap-3`}>
          <motion.div 
            ...
            style={{
              background: `linear-gradient(135deg, ${activeDomainColor} 0%, ${activeDomainColor}dd 50%, ${activeDomainColor}bb 100%)`,
            }}
          ...
          <div className="px-3 py-2">
            ...
            >
              <div className="flex items-center gap-2">
                <Filter size={12} />
                <span className="font-medium">{DOMAINS.find(d => d.id === activeDomain)?.name}</span>
              </div>
              <ChevronDown size={12} className={`transition-transform ${showDomainFilter ? 'rotate-180' : ''}`} />
            </motion.button>
          ...
          {showExport && (
            <motion.div
              ...
              <button onClick={() => { void exportPNG() }}
                ...
                style={{ borderColor: `${activeDomainColor}40`, color: activeDomainColor, backgroundColor: `${activeDomainColor}15` }}>
...
const NavButton = ({ item, active, onClick, collapsed, dark, activeColor = '#6366f1' }: NavButtonProps) => {
