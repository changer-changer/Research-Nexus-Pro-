...
function App() {
  const { activeView, setActiveView, viewConfig, updateViewConfig, loadData, expandAll, undo, redo } = useAppStore()
  ...
  useEffect(() => {
    import('./data/real_papers.json').then((data) => {
      ...
      // Expand first 2 levels only (root + sub-problems)
      setTimeout(() => {
        const store = useAppStore.getState()
        const level2Ids = store.problems.filter(p => (p.depth || 0) <= 1).map(p => p.id)
        store.methods.filter(m => !m.parentId).forEach(m => level2Ids.push(m.id))
        useAppStore.setState({ expandedNodes: new Set(level2Ids) })
      }, 100)
    })
  }, [])
